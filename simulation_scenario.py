...
class SimulationAudioManager:
    ...
    async def run(self):
        ...
        for item in self.script_data:
            ...
            # Updates occur on even indices (or custom logic)
            if index % 2 == 0:
                # Determine file index logic (0, 1, 2...)
                update_index = max(0, int(index // 2))

                # Define the updates we want to check and send
                # Format: (Folder Name, File Prefix, WS Message Type, JSON Data Key)
                updates_to_process = [
                    ("questions", "q", "questions", "questions"),
